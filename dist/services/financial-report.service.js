...
class FinancialReportService {
    ...
    async getSalesByCategory(filters) {
        ...
        return categories.map(category => {
            const totalSold = category.dataItems.reduce((sum, product) => sum + product.orderItems.reduce((pSum, item) => pSum + item.quantity, 0), 0);
            const revenue = category.dataItems.reduce((sum, product) => sum + product.orderItems.reduce((pSum, item) => pSum + (item.price * item.quantity), 0), 0);
            return {
                id: category.id,
                name: category.name,
                totalSold,
    ...
    async exportReportToCSV(filters) {
    ...
    async getRecentTransactions(startDate, endDate, limit = 20) {
