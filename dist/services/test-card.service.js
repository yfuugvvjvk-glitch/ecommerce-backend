...
exports.TestCardService = void 0;
const client_1 = require("@prisma/client");
const prisma = new client_1.PrismaClient();
class TestCardService {
    // DOAR ADMINII pot crea carduri de test
    async createTestCard(adminId, cardData) {
    ...
    async validateTestCard(cardNumber, cvv, expiryMonth, expiryYear) {
    ...
    async processTestCardPayment(userId, orderId, cardData) {
        const validation = await this.validateTestCard(cardData.cardNumber, cardData.cvv, cardData.expiryMonth, cardData.expiryYear);
        if (!validation.valid) {
    ...
    async refundTestCardPayment(orderId) {
    ...
    async updateTestCard(adminId, cardId, updateData) {
        const admin = await prisma.user.findUnique({
            where: { id: adminId },
            select: { role: true }
        });
    ...
    async deleteTestCard(adminId, cardId) {
        const admin = await prisma.user.findUnique({
            where: { id: adminId },
            select: { role: true }
        });
    ...
    async generateDefaultTestCards(adminId) {
        const admin = await prisma.user.findUnique({
            where: { id: adminId },
            select: { role: true }
        });
    ...
    async saveTestCardForUser(userId, testCardId) {
...
exports.TestCardService = TestCardService;
