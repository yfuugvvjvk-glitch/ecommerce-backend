...
class CartService {
    ...
    async getCart(userId) {
        ...
        const total = itemsWithParsedQuantities.reduce((sum, item) => {
    ...
    // Add item to cart
    async addToCart(userId, dataItemId, quantity = 1) {
     ...
     */
    async addGiftProduct(userId, giftRuleId, productId) {
        ...
        const validation = await giftValidator.validateGiftSelection(userId, giftRuleId, productId, currentCart);
     ...
     */
    async removeGiftProduct(userId, cartItemId) {
        const cartItem = await prisma.cartItem.findFirst({
     ...
     */
    async reevaluateGifts(userId) {
        // Import services (lazy to avoid circular dependencies)
        const { conditionEvaluator } = await Promise.resolve().then(() => __importStar(require('./condition-evaluator.service')));
        const { giftRuleService } = await Promise.resolve().then(() => __importStar(require('./gift-rule.service')));
        ...
        for (const giftItem of giftItems) {
            ...
            const result = await conditionEvaluator.evaluateRule(rule, context);
