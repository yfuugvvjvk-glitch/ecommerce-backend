...
class AdminService {
    ...
    async getDashboardStats() {
        const [totalUsers, totalProducts, totalOrders, totalRevenue, pendingOrders, recentOrders,] = await Promise.all([
            prisma.user.count(),
    ...
    async approveVoucherRequest(requestId) {
    ...
    async rejectVoucherRequest(requestId) {
        const request = await prisma.voucherRequest.findUnique({
            where: { id: requestId },
        });
        if (!request) {
    ...
    async resetVoucherRequest(requestId) {
        const request = await prisma.voucherRequest.findUnique({
            where: { id: requestId },
        });
        if (!request) {
    ...
    async updateVoucherRequest(requestId, data) {
        return await prisma.voucherRequest.update({
            where: { id: requestId },
            data: {
                code: data.code,
