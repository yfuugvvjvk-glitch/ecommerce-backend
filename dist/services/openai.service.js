...
Suntem disponibili în programul magazinului fizic sau ne poți scrie oricând!`;
        ...
        if (message.includes('livrare') || message.includes('livrez') || message.includes('transport') || message.includes('curier')) {
            try {
                const deliveryMethods = await prisma_1.prisma.deliverySettings.findMany({
                    where: { isActive: true },
                });
                if (deliveryMethods.length > 0) {
                    ...
                    const locations = await prisma_1.prisma.deliveryLocation.findMany({
                        where: { isActive: true },
                        take: 3,
                    });
...
💡 Cursurile sunt actualizate de la surse oficiale pentru acuratețe maximă!`;
        ...
        if (message.includes('fermă') || message.includes('ferma') || message.includes('unde sunteți') || message.includes('locație') || message.includes('adresă')) {
            try {
                const mainLocation = await prisma_1.prisma.deliveryLocation.findFirst({
...
📱 0753615742`;
    ...
    async getPlatformContext() {
        try {
            const [totalProducts, totalOrders, recentOrders, categories, activeOffers, activeVouchers, topProducts, deliveryMethods, paymentMethods, deliveryLocations, siteConfig, pages, giftRules,] = await Promise.all([
                prisma_1.prisma.dataItem.count({ where: { status: 'published' } }),
                ...
                prisma_1.prisma.deliverySettings.findMany({
                    where: { isActive: true },
                }),
                prisma_1.prisma.paymentMethod.findMany({
                    where: { isActive: true },
                }),
                prisma_1.prisma.deliveryLocation.findMany({
                    where: { isActive: true },
                    take: 10,
                }),
                prisma_1.prisma.siteConfig.findMany(),
                prisma_1.prisma.page.findMany({
                    where: { isPublished: true },
                    select: { slug: true, title: true, metaDescription: true },
                }),
                prisma_1.prisma.giftRule.findMany({
                    where: { isActive: true },
                    take: 5,
                }),
            ]);
            ...
            if (categories.length > 0) {
                ...
                categories.forEach((cat) => {
                    ...
                    if (cat.subcategories && cat.subcategories.length > 0) {
                        context += ` (Subcategorii: ${cat.subcategories.map((sub) => sub.name).join(', ')})`;
                    }
                    context += '\n';
                });
                context += '\n';
