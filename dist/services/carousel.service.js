...
class CarouselService {
    ...
    async getAllCarouselItems(includeInactive = false) {
    ...
    async isPositionAvailable(position, excludeId) {
    ...
    async createCarouselItem(data, userId) {
    ...
    async swapPositions(itemId1, itemId2) {
    ...
    async moveToPosition(itemId, newPosition) {
        const item = await prisma.carouselItem.findUnique({
            where: { id: itemId }
        });
        if (!item) {
...
exports.carouselService = new CarouselService();
