...
class ChatService {
    ...
    async createGroupChat(creatorId, name, description, memberIds = []) {
        try {
    ...
    // Trimite un mesaj
    async sendMessage(chatRoomId, senderId, content, type = 'TEXT', fileUrl, fileName) {
        try {
    ...
    async getChatMessages(chatRoomId, userId, page = 1, limit = 50) {
        try {
            // Verifică dacă utilizatorul este membru al camerei
            const membership = await prisma.chatRoomMember.findFirst({
                where: {
    ...
    async addMembersToGroup(chatRoomId, adminId, memberIds) {
        try {
    ...
    // Părăsește un grup
    async leaveGroup(chatRoomId, userId) {
        try {
            const membership = await prisma.chatRoomMember.findFirst({
                where: {
                    chatRoomId,
    ...
    async getNewChatUsers(userId) {
        try {
            ...
            const existingChatUserIds = existingDirectChats.flatMap(chat => chat.members.map(member => member.userId));
    ...
    async deleteConversation(chatRoomId, userId) {
        try {
            // Verifică dacă utilizatorul este membru al camerei
            const membership = await prisma.chatRoomMember.findFirst({
                where: {
